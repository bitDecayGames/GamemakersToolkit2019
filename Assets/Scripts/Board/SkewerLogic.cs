...
public static class SkewerLogic {
    ...
    public static GameOverStatus ShootSkewer(BoardStep boardStep, Vector2 direction, SkewerThrower thrower) {
        int curDirX = 0;
        int curDirY = 0;
        ...
        for (int row = 0; row < boardStep.Rows(); row++) {
            for (int column = 0; column < boardStep[row].Count; column++) {
                var node = boardStep[row][column];
                if (node.entity != null) {
        ...
        while (curPosY + curDirY < boardStep.Rows() && curPosY + curDirY >= 0 && curPosX + curDirX < boardStep[curPosY].Count && curPosX + curDirX >= 0) {
            // this is the next step
            curPosX += curDirX;
            curPosY += curDirY;

            var node = boardStep[curPosY][curPosX];
            if (node.entity != null) {
                var entity = node.entity.GetComponent<Entity>();
        ...
        var lastValidNode = boardStep[curPosY][curPosX];
